...
def buy_stock(symbol: str, quantity: int, total_price: float) -> str:
    '''Buy a stock given the stock symbol and quantity
       :param symbol: stock symbol
       :param quantity: number of shares to buy
       :param total_price: total price of the stock
       :return: confirmation message
       '''
    decison = interrupt(f"Approve buying  {quantity} shares of {symbol} for a total price of {total_price:.2f}?")
    if decison == "yes":
        return f"You Bought {quantity} shares of {symbol} for a total price of {total_price}."
    else:
        return "Buying declined"

class HumanLoopChatBot:
    __llm_with_tools = None
    __graph = None
    __tools = [get_stock_price, buy_stock]
    ...
    def run(self):
        ...
        while True:
            ...
            if result.get('__interrupt__'):
                decision = input("Press yes to Buy\n")
                result = self.__graph.invoke(Command(resume=decision), config=config)
